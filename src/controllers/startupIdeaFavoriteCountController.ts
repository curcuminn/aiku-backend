...
export const getFavoriteCount: RequestHandler = async (req, res) => {
  try {
    ...
    if (!ideaId)
      return res
        .status(400)
        .json({ success: false, message: "ideaId gerekli" });
...
export const incrementFavoriteCount: RequestHandler = async (req, res) => {
  try {
    ...
    if (delta < 0) {
      return res
        .status(400)
        .json({ success: false, message: "delta 0'dan büyük olmalı" });
    }

    const doc = await StartupIdeaFavoriteCount.increment(ideaId, delta);
    return res
      .status(200)
      .json({ success: true, data: { ideaId, count: doc.count } });
  } catch (err: any) {
    return res
      .status(500)
      .json({ success: false, message: "Sunucu hatası", error: err.message });
...
export const setFavoriteCount: RequestHandler = async (req, res) => {
  try {
    ...
    if (!Number.isFinite(value) || value < 0) {
      return res
        .status(400)
        .json({ success: false, message: "value 0 veya pozitif olmalı" });
    }
    const doc = await StartupIdeaFavoriteCount.setCount(
      ideaId,
      Math.floor(value)
    );
    return res
...
export const getFavoriteCountsBulk: RequestHandler = async (req, res) => {
  try {
    ...
    if (!ids.length) {
      return res
        .status(400)
        .json({ success: false, message: "ids boş olamaz" });
    }
    const rows = await StartupIdeaFavoriteCount.find({ ideaId: { $in: ids } })
      .select({ ideaId: 1, count: 1, _id: 0 })
      .lean()
      .exec();
...
export const favoriteIdea: RequestHandler = async (req, res) => {
  try {
    ...
    const auth = (req.get("authorization") || req.headers["authorization"]) as
      | string
      | undefined;
    ...
    if (already) {
      currentUser.favoriteIdeas = currentUser.favoriteIdeas.filter(
        (i) => i !== idStr
      );
      ...
      return res.status(200).json({
        success: true,
        data: { ideaId: idStr, status: "removed", count: newCount },
      });
    } else {
      ...
      return res.status(200).json({
        success: true,
        data: { ideaId: idStr, status: "added", count: doc.count },
      });
    }
  } catch (err: any) {
    return res
