...
interface IStartupIdeaFavoriteCountModel extends Model<IStartupIdeaFavoriteCount> {
...
// ---- Statics ----
StartupIdeaFavoriteCountSchema.statics.getCount = async function (ideaId: string) {
    const doc = await this.findOne({ ideaId }).lean().exec();
    return doc?.count ?? 0;
};

StartupIdeaFavoriteCountSchema.statics.increment = function (ideaId: string, delta = 1) {
...
StartupIdeaFavoriteCountSchema.statics.setCount = function (ideaId: string, value: number) {
